...
function normalizeWixLineItem(raw: Record<string, unknown>): PrintOpsNormalizedLineItem {
  ...
  return {
    ...
    sku: firstString([raw.sku, readPath(raw, "catalogReference.sku"), readPath(raw, "product.sku")]),
    barcode: firstString([raw.barcode, raw.productBarcode, readPath(raw, "catalogReference.barcode"), readPath(raw, "product.barcode")]),
...
function normalizeCustomer(raw: Record<string, unknown>): PrintOpsNormalizedCustomer {
  ...
  return {
    ...
    email: firstString([readPath(record ?? {}, "email"), readPath(raw, "buyerInfo.email"), readPath(raw, "billingInfo.contactDetails.email")]),
    phone: firstString([readPath(record ?? {}, "phone"), readPath(record ?? {}, "phoneNumber"), readPath(raw, "buyerInfo.phone"), readPath(raw, "billingInfo.contactDetails.phone")]),
