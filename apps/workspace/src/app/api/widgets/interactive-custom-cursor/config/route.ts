...
import { getInteractiveCursorWixSecretFromDatabase, parseWixInstance } from "@/lib/wix/app-instance";
...
export async function POST(request: NextRequest) {
  ...
  const instanceId = await resolveWritableInstanceId(body.platform, body.instanceId, body.instanceToken);
...
async function resolveWritableInstanceId(platform: CursorConfigRecord["platform"], instanceId?: string, instanceToken?: string) {
  if (platform !== "wix") {
    return instanceId ?? null;
  }

  const parsed = parseWixInstance(instanceToken, await getInteractiveCursorWixSecretFromDatabase());
