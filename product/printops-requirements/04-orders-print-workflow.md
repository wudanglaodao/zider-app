...
- 支持连接 Wix Stores。
- 支持一个商家组织下连接多个 Wix 店铺的数据结构预留；当前 UI 先按当前 Wix instance 工作。
- 支持最新订单同步：默认同步最近 3 天订单。
- 支持最近 7 天历史订单手动同步，用于新安装店铺补齐近期订单和测试模板；该操作放在更多菜单，不作为常驻主按钮。
- 不支持超过 7 天的历史回溯；如果商家需要更久历史，先作为 P1 / P2 导入能力评估。
- 支持拉取订单列表和订单详情。
- 支持保存平台原始订单 payload。
- 支持将原始 payload 转换为标准订单模型，并保留订单级和订单行级自定义字段。
- 支持记录订单同步时间。
- 支持订单列表每 3 分钟从 PrintOps 缓存刷新一次。
- 支持同步失败提示。
- 支持订单按店铺隔离。
- 支持 Wix backend extension 将订单业务事件转发到 PrintOps webhook，事件入库后更新订单缓存。
...
- 默认只显示当前店铺订单。
- 支持按打印状态筛选：Printed、Unprinted。
- 支持搜索订单号、客户名、邮箱、SKU。
- 支持批量选择订单。
- 显示每单推荐模板和打印语言。
- 切换店铺后清空已选订单。
- 不在列表常驻展示渠道列，但订单数据必须保存平台、instance 和 store 上下文，后续多店铺筛选时使用。
- 列表字段必须来自真实同步订单，不允许展示 `Wix customer`、`Wix order item` 等 mock 文案。
...
- 支持单订单预览。
- 支持选中订单后进入批量打印面板；列表页不常驻展示批量预览区域。
- 预览使用真实纸张尺寸。
- 支持 A4；Letter、4x6、80mm 作为后续尺寸预留。
- 校验必填字段缺失。
- 校验图片无法加载。
- 校验内容溢出页面。
- 校验当前字体可能缺字。
...
- 支持失败订单单独提示。
- 支持按模板和语言生成打印结果。
- 批量 PDF、浏览器打印和标记已打印必须只作用于当前选中订单。
- 批量打印不能跨 store 执行。
- 单行操作支持 PDF、Print、Preview icon 和更多菜单。
- 打印订单永远使用数据库里的当前默认模板语言和设置。
...
每次生成、下载或浏览器打印，都需要创建 print job 或可追溯的打印状态记录。
...
- Orders 列表显示 Printed / Unprinted 状态。
- 批量 `Mark as printed` 会更新选中订单打印状态。
- Browser print 只能表示已发送到浏览器打印，不保证实体打印成功。
- 独立 Print History 页面暂缓，后续再将打印状态升级为完整 print job history。
...
1. 商家进入 Orders。
2. 使用 `Unprinted` 筛选待打印订单；支付和履约状态作为列表字段辅助判断。
3. 勾选多个订单。
4. 系统使用当前店铺数据库中的默认 `Invoice - A4` 模板。
5. 系统按默认模板语言选择打印语言。
6. 系统执行字段、图片、字体、分页校验。
7. 商家查看异常提示。
8. 商家确认生成 PDF。
9. 系统创建 print job 或打印状态记录。
